...
app.post('/api/createmitarbeiter', async (req, res) => {
    ...
    try {
        let result = await DatabaseQueries.executeQuery("INSERT INTO MITARBEITER (MITARBEITERID, FIRMENID, VORNAME, NACHNAMEN, GEBURTSDATUM, STUNDENLOHN, IBAN, VERSICHERTENNUMMER, URLAUBSTAGE, ABTEILUNG, EINSTELLUNGSJAHR, AZUBI, FueHRERSCHEIN, TELEFON) \
            VALUES (:MITARBEITERID, :FIRMENID, :VORNAME, :NACHNAMEN, :GEBURTSDATUM, :STUNDENLOHN, :IBAN, :VERSICHERTENNUMMER, :URLAUBSTAGE, :ABTEILUNG, :EINSTELLUNGSJAHR, :AZUBI, :FueHRERSCHEIN, :TELEFON)",
            { MITARBEITERID: mitarbeiter.MITARBEITERID, FIRMENID: mitarbeiter.FIRMENID, VORNAME: mitarbeiter.VORNAME, NACHNAMEN: mitarbeiter.NACHNAMEN, GEBURTSDATUM: mitarbeiter.GEBURTSDATUM, STUNDENLOHN: mitarbeiter.STUNDENLOHN, IBAN: mitarbeiter.IBAN, VERSICHERTENNUMMER: mitarbeiter.VERSICHERTENNUMMER, URLAUBSTAGE: mitarbeiter.URLAUBSTAGE, ABTEILUNG: mitarbeiter.ABTEILUNG, EINSTELLUNGSJAHR: mitarbeiter.EINSTELLUNGSJAHR, AZUBI: mitarbeiter.AZUBI, FueHRERSCHEIN: mitarbeiter.FueHRERSCHEIN, TELEFON: mitarbeiter.TELEFON });
        res.status(200).json({ success: true });
    } catch (err) {
        console.log(err);
        res.status(500).json({ success: false });
